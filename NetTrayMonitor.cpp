...
void _P() { HWND t = FindWindow(L"Shell_TrayWnd", 0); if (!t)return; HWND n = FindWindowEx(t, 0, L"TrayNotifyWnd", 0); if (!n)return; RECT r; GetWindowRect(n, &r); POINT p = { r.left,r.top }; ScreenToClient(t, &p); HDC c = GetDC(0); int d = GetDeviceCaps(c, 90); ReleaseDC(0, c); int s = MulDiv(40, d, 96); int a = min((int)(r.bottom - r.top), s); _W(_h, a); if (GetParent(_h) != t) { SetParent(_h, t); SetWindowLong(_h, -16, (GetWindowLong(_h, -16) & ~0x80000000) | 0x40000000); } int mx = p.x - _cw - 6; SetWindowPos(_h, HWND_TOP, mx, p.y, _cw, a, 0x0010 | 0x0040); HWND rb = FindWindowEx(t, 0, L"ReBarWindow32", 0); if (rb) { RECT rr; GetWindowRect(rb, &rr); POINT rp = { rr.left, rr.top }; ScreenToClient(t, &rp); int tw = (mx - 10) - rp.x; if (tw > 0 && (rr.right - rr.left) != tw) { SetWindowPos(rb, 0, 0, 0, tw, rr.bottom - rr.top, 0x0002 | 0x0004 | 0x0010); } } }
