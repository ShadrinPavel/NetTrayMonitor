...
void _R() { HWND t = FindWindow(L"Shell_TrayWnd", 0); if (t) PostMessage(t, WM_SIZE, 0, 0); }

void _P() { HWND t = FindWindow(L"Shell_TrayWnd", 0); if (!t)return; HWND n = FindWindowEx(t, 0, L"TrayNotifyWnd", 0); if (!n)return; RECT r; GetWindowRect(n, &r); POINT p = { r.left,r.top }; ScreenToClient(t, &p); HDC c = GetDC(0); int d = GetDeviceCaps(c, 90); ReleaseDC(0, c); int s = MulDiv(40, d, 96); int a = min((int)(r.bottom - r.top), s); _W(_h, a); if (GetParent(_h) != t) { SetParent(_h, t); SetWindowLong(_h, -16, (GetWindowLong(_h, -16) & ~0x80000000) | 0x40000000); } int mx = p.x - _cw - 6; SetWindowPos(_h, HWND_TOP, mx, p.y, _cw, a, 0x0010 | 0x0040); HWND rb = FindWindowEx(t, 0, L"ReBarWindow32", 0); if (rb) { RECT rr; GetWindowRect(rb, &rr); POINT rp = { rr.left, rr.top }; ScreenToClient(t, &rp); int tw = (mx - 10) - rp.x; int th = r.bottom - r.top; if (tw > 0 && ((rr.right - rr.left) != tw || (rr.bottom - rr.top) != th)) { SetWindowPos(rb, 0, 0, 0, tw, th, 0x0002 | 0x0004 | 0x0010); } } }

LRESULT CALLBACK _WND(HWND h, UINT m, WPARAM w, LPARAM l) { switch (m) { case 1:SetTimer(h, 1, 1000, 0); _U(); break; case 275:if (!FindWindow(L"Shell_TrayWnd", 0))PostQuitMessage(0); _U(); _P(); SetLayeredWindowAttributes(h, _L() ? RGB(255, 255, 255) : 0, 0, 1); InvalidateRect(h, 0, 0); break; case 0x0205: { POINT pt; GetCursorPos(&pt); HMENU hm = CreatePopupMenu(); InsertMenu(hm, 0, MF_BYPOSITION | MF_STRING, 1001, L"Перезапустить Монитор"); InsertMenu(hm, 1, MF_BYPOSITION | MF_STRING, 1002, L"Закрыть Монитор"); SetForegroundWindow(h); int cmd = TrackPopupMenu(hm, 0x0100 | 0x0002, pt.x, pt.y, 0, h, 0); DestroyMenu(hm); if (cmd == 1001) { wchar_t p[MAX_PATH]; GetModuleFileName(0, p, MAX_PATH); ShellExecute(0, L"open", p, 0, 0, 1); PostQuitMessage(0); } else if (cmd == 1002) { PostQuitMessage(0); } break; } case 20:return 1; case 15: { PAINTSTRUCT p; HDC c = BeginPaint(h, &p); RECT r; GetClientRect(h, &r); HDC m = CreateCompatibleDC(c); HBITMAP b = CreateCompatibleBitmap(c, r.right, r.bottom); HGDIOBJ o = SelectObject(m, b); bool l_ = _L(); COLORREF k = l_ ? RGB(255, 255, 255) : 0; HBRUSH r_ = CreateSolidBrush(k); FillRect(m, &r, r_); DeleteObject(r_); COLORREF c1 = l_ ? 0 : RGB(255, 255, 255); COLORREF c2 = l_ ? RGB(100, 100, 100) : RGB(160, 160, 160); int f = (r.bottom - 10) / 2; HFONT h1 = CreateFont(f, 0, 0, 0, 700, 0, 0, 0, 1, 0, 0, 4, 0, L"Segoe UI Variable"); HFONT h2 = CreateFont(f, 0, 0, 0, 400, 0, 0, 0, 1, 0, 0, 4, 0, L"Segoe UI"); SetBkMode(m, 1); wchar_t bi[64], bo[64]; _F(_si, bi); _F(_so, bo); int y1 = 4, y2 = 6 + f; SIZE a; SetTextColor(m, c1); SelectObject(m, h1); TextOut(m, 4, y1, L"\x2193 ", 2); GetTextExtentPoint32(m, L"\x2193 ", 2, &a); SelectObject(m, h2); TextOut(m, 4 + a.cx, y1, bi, lstrlen(bi)); SetTextColor(m, c2); SelectObject(m, h1); TextOut(m, 4, y2, L"\x2191 ", 2); GetTextExtentPoint32(m, L"\x2191 ", 2, &a); SelectObject(m, h2); TextOut(m, 4 + a.cx, y2, bo, lstrlen(bo)); BitBlt(c, 0, 0, r.right, r.bottom, m, 0, 0, 13369376); SelectObject(m, o); DeleteObject(b); DeleteDC(m); DeleteObject(h1); DeleteObject(h2); EndPaint(h, &p); break; }case 2:_R(); PostQuitMessage(0); break; default:return DefWindowProc(h, m, w, l); }return 0; }
