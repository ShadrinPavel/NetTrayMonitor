...
> [!NOTE]
> **Архитектурная спецификация:** Утилита работает без прав Администратора (Portable-режим) и считывает аппаратные счётчики активного сетевого адаптера на канальном уровне (L2). Это означает, что отображаемая скорость включает в себя **весь трафик интерфейса** (как внешний Интернет, так и локальный трафик от LAN-устройств, например, ONVIF-камер или NAS). Разделение трафика программным путём без прав Администратора в ОС Windows невозможно из соображений безопасности.

## История версий
* **v1.2.3** — Обновление спецификаций проекта. Добавлено техническое обоснование L2-мониторинга (учёт локального LAN-трафика) при работе в режиме без UAC.
* **v1.2.2** — Исправлен баг Windows с проваливанием иконок (word-wrap ReBarWindow32). Добавлено контекстное меню (ПКМ). Переход на нативную архитектуру x64 (WinPE 11 совместимость).
* **v1.2.1** — Внедрена кастомная мультиразмерная иконка (256x256). Подтверждена и зафиксирована 100% совместимость со всеми разрядностями (Win32/x86/WOW64) начиная с Windows 7.
